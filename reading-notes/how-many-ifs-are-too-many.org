...
CPU执行上面代码的逻辑，大概是这样的：
cycle 1：CPU前端，读取BR指令，此时BR还没有解码;
cycle 2：CPU前端有两个选择，等待BR的结果（branch target），或者取下一条指令（希望它是branch target）; 此时BR完成解码;
cycle 3：CPU前端，取下一条指令（X1）; 后端执行BR指令;
cycle 4：CPU后端完成BR指令的执行，前端知道自己取错指令了。

在这个过程中，前端浪费了2个cycles（3和4）。分支越复杂，前端浪费的cycles越多。所以需要分支预测。
