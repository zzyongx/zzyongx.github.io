...
** Intention Locks (意向锁)
Intention Locks是表级锁，但不一定锁整个表，如果where字句有索引的化，。通过 ~SELECT ... LOCK IN SHARE MODE~ 和 ~SELECT ... FOR UPDATE~ 加Intention Locks。前者加 =IS= 锁，后者加 =IX= 锁，他们和 =S= 和 =X= 的兼容情况时：
|    | X        | IX         | S          | IS         |
...
ERROR 1205 (HY000): Lock wait timeout...      |    -- 加Intention Locks
                                              |    SELECT * FROM testt SET n >= 8 FOR UPDATE;
-- 因为Intention Lock是表锁，插入失败         |
